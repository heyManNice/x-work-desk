...
/* 实体机正登录的所有用户名（seat0 class=user，去重）。返回数量/-1 */
int localsess_seat_users(char users[][64], int max)
{
    if (guard_forced())
        return 0;
    if (geteuid() != 0)
    ...
    {
        char out[2048];
        show_session(all[i], out, sizeof out);
        if (!out[0])
            continue;
        char seat[64], cls[64], name[64];
        if (!prop_get(out, "Seat", seat, sizeof seat) || strcmp(seat, SEAT))
            continue;
        if (!prop_get(out, "Class", cls, sizeof cls) || strcmp(cls, "user"))
            continue;
        const char *nm = prop_get(out, "Name", name, sizeof name);
        if (!nm && !(nm = prop_get(out, "User", name, sizeof name)))
            continue;
        int dup = 0;
        for (int j = 0; j < m; j++)
            if (!strcmp(users[j], nm))
            {
                dup = 1;
                break;
            }
